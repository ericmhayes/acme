...
Two infrastructure hardening items fall out of this same compliance event rather
than firing on their own signal, so they sit under it:

**1a. Tighter, per-resource apply IAM (least privilege).** **When** SOC 2 begins
(or the team grows past ~6 engineers and more people can trigger applies) — **we
need** to replace the service-scoped Terraform apply policy with per-resource least
privilege and a permissions boundary. Today the apply role is scoped to the
services this stack manages, which is honest but broad; hand-tuning it is
high-effort and low-value until an auditor or a larger team raises the
blast-radius stakes.

**1b. Customer-managed KMS keys + egress control.** **When** a customer contract,
BAA amendment, or auditor explicitly requires key custody or demonstrable egress
restriction — **we need** customer-managed KMS keys for RDS/Secrets/logs and an
egress control (AWS Network Firewall or equivalent). AWS-managed encryption already
covers encryption-at-rest; this is about *who holds the keys* and *proving* egress
is constrained.
...
### 3. PR / ephemeral preview environments
...
### 4. Read replica + a separate reporting/analytics path
